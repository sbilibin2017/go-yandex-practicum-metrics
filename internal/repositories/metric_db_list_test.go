...
func setupListPostgresContainer(t *testing.T) (*sql.DB, func()) {
...
func TestMetricDBListRepository_List(t *testing.T) {
	db, cleanup := setupListPostgresContainer(t)
	defer cleanup()
	repo := repositories.NewMetricDBListRepository(db)
	...
	metric3, exists := result[domain.MetricID{ID: "metric3", Type: "counter"}]
	assert.True(t, exists)
	assert.Equal(t, int64(5), *metric3.Delta)
	assert.Nil(t, metric3.Value)
}
